...
RETURNS TEXT AS $$
  SELECT role FROM public.users WHERE id = auth.uid()::integer;
$$ LANGUAGE SQL SECURITY DEFINER STABLE SET search_path = public;

-- Create function to get current user school
CREATE OR REPLACE FUNCTION public.get_current_user_school()
RETURNS INTEGER AS $$
  SELECT school_id FROM public.users WHERE id = auth.uid()::integer;
$$ LANGUAGE SQL SECURITY DEFINER STABLE SET search_path = public;

-- Create function to get student's user_id
CREATE OR REPLACE FUNCTION public.get_student_user_id(student_id_param INTEGER)
RETURNS INTEGER AS $$
  SELECT user_id FROM public.students WHERE id = student_id_param;
$$ LANGUAGE SQL SECURITY DEFINER STABLE SET search_path = public;
...
CREATE POLICY "Students can view their class" ON public.classes
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.students s WHERE s.class_id = id AND s.user_id = auth.uid()::integer)
  );
...
CREATE POLICY "Students can view their own papers" ON public.exam_papers
  FOR SELECT USING (student_id IN (
    SELECT id FROM public.students WHERE user_id = auth.uid()::integer
  ));
